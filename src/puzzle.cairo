...
// exercise
func verify_location_is(loc: Location*, row: felt, col: felt) {
    assert loc.row = row;
    assert loc.col = col;
    return ();
}
...
func verify_location_list(loc_list: Location*, n_steps) {
    ...
    if (n_steps == 0) {
        return verify_location_is(loc_list, 3, 3);
    }

    verify_adjacent_locations(loc_list, loc_list + Location.SIZE);

    return verify_location_list(loc_list + Location.SIZE, n_steps - 1);
}
